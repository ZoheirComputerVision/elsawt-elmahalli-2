...
export const newsService = {
  ...
  async submitForReview(id: string): Promise<NewsResponse> {
    const item = await newsRepository.update(id, { status: "review" } as Prisma.NewsUpdateInput);
    return toNewsResponse(item);
  },

  async approve(id: string): Promise<NewsResponse> {
    const item = await newsRepository.update(id, { status: "approved" } as Prisma.NewsUpdateInput);
    return toNewsResponse(item);
  },
